...
function fix(node, pathToFile, type) {
  return function(fixer) {
    const value = node.value;
    const token =
      value.length < 20
        ? value
        : `${extractFilename(pathToFile)}${hash(node.value)}`;

    if (type === 'Literal' && node.parent.type === 'JSXExpressionContainer') {
      return fixer.replaceText(node, ` t(\`${token}\`, \`${value}\`) `);
    } else {
      return fixer.replaceText(node, `{ t(\`${token}\`, \`${value}\`) }`);
    }
  };
  ...
  create(context) {
    ...
    function validateLiteralNode(node) {
      ...
      context.report({
        node,
        message,
        fix: fix(node, context.getFilename(), 'Literal'),
      });
    }
    ...
    const scriptVisitor = {
      ...
      JSXText(node) {
        ...
        context.report({
          node,
          message,
          fix: fix(node, context.getFilename(), 'JSXText'),
        });
      },
      // ─────────────────────────────────────────────────────────────────
      ...
      TemplateLiteral(node) {
        ...
        quasis.some(({ value: { raw } }) => {
          ...
          context.report({
            node,
            message,
            fix: fix(node, context.getFilename(), 'TemplateLiteral'),
          });
          return true; // break
        });
      },
